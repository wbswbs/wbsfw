...
class Smtp extends WbsClass
{
     ...
     * @throws Exception
     */
    public function sendMailToSiteAdmin(
                  $subject,
        $body,
     ...
     * @throws Exception
     */
    public function sendMail(string|array $recipient, string $subject, string $body)
    {
        $this->wbs()->log()->info('[SMTP]' . __FUNCTION__ . '() To: ' .$recipient. ' Subject: ' . $subject);
...
//            ];
     ...
     * @throws Exception
     */
    public function sendMailByData(array $data = [])
    {
            $mail          = new PHPMailer(true); // create a new object
            $mail->CharSet = 'UTF-8';
            $mail->IsSMTP(
            ...
            $mail->Username   = $this->user;
            $mail->Password   = $this->pass;
            $mail->SetFrom($data['recipient'] ?? "noreply@wbs.de");
